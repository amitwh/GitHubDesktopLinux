...
async function handleCommandLineArguments(argv: string[]) {
  ...
  // Scan argv for an OAuth / app URL passed by the OS via the .desktop
  // file's Exec=...%U (Linux) or `--protocol-launcher` flag (Windows).
  // On Linux the system invokes the registered .desktop entry and the
  // URL arrives as a plain argv entry (e.g. when a browser opens
  // x-github-desktop-auth://callback?code=...). On Windows, Chromium
  // may interleave `--protocol-launcher` and other switches.
  ...
  if (matchingUrl) {
    ...
    // On Windows, bail out so a smuggled cli switch can't be acted on.
    if (__WIN32__) {
      return
    }
  } else if (__WIN32__ && args['protocol-launcher'] === true) {
